...
const unsigned char larimar_cyan_highlight[] = {
        0x2E, 0xD0, 0xE5, 0xFF, 0x2E, 0xD0, 0xE5, 0xFF, 0x2E, 0xD0, 0xE5, 0xFF, 0x2E, 0xD0, 0xE5, 0xFF
};
const unsigned char miiverse_green_touch1[] = {
        0x94, 0xd9, 0x2a, 0x00, 0x57, 0xbd, 0x12, 0xff
};
const unsigned char larimar_cyan_touch1[] = {
        0x2E, 0xD0, 0xE5, 0x00, 0x00, 0xB4, 0xC8, 0xFF
};
const unsigned char miiverse_green_touch2[] = {
        0x57, 0xbd, 0x12, 0x00, 0x94, 0xd9, 0x2a, 0xff
};
const unsigned char larimar_cyan_touch2[] = {
        0x00, 0xB4, 0xC8, 0x00, 0x2E, 0xD0, 0xE5, 0xFF
};

const replacement replacements[] = {
        {miiverse_green_highlight, larimar_cyan_highlight},
        {miiverse_green_touch1,    larimar_cyan_touch1},
        {miiverse_green_touch2,    larimar_cyan_touch2},
};
...
DECL_FUNCTION(int, FSOpenFile, FSClient *client, FSCmdBlock *block, char *path, const char *mode, uint32_t *handle,
    ...
    if (!Config::connect_to_network) {
        DEBUG_FUNCTION_LINE_VERBOSE("Inkay-Larimar: Miiverse patches skipped.");
    ...
    if (strcmp(initialOma, path) == 0) {
        ...
        DEBUG_FUNCTION_LINE_VERBOSE("Inkay-Larimar: Found Miiverse CA, replacing...");
        return ret;
    }
...
DECL_FUNCTION(FSStatus, FSReadFile, FSClient *client, FSCmdBlock *block, uint8_t *buffer, uint32_t size, uint32_t count,
    ...
    if (size != 1) {
        DEBUG_FUNCTION_LINE("Inkay-Larimar: Miiverse CA replacement failed!");
    }
...
void patchOlvApplet() {
    ...
    auto add_patch = [](function_replacement_data_t repl, const char *name) {
        ...
        if (FunctionPatcher_AddFunctionPatch(&repl, &handle, nullptr) != FUNCTION_PATCHER_RESULT_SUCCESS) {
            DEBUG_FUNCTION_LINE("Inkay-Larimar/OLV: Failed to patch %s!", name);
        }
